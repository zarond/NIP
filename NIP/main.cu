...
void render()
{
	...
	case 0:
		...
		timer.toc();
		cudaMemcpy(d_Field, Field.data() + M * N * (time_ind % NFrames), M * N * sizeof(Real), cudaMemcpyHostToDevice);
		break;
	case 1:
		timer.tic();
		WaveIterationOMP(Field, IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, F, b, Threads);
		timer.toc();
		cudaMemcpy(d_Field, Field.data() + M * N * (time_ind % NFrames), M * N * sizeof(Real), cudaMemcpyHostToDevice);
		break;
	case 2:
		timer.tic();
		WaveIterationOMPMultipleFrames(Field, IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, F, b, iter, Threads);
		timer.toc(iter);
		time_ind += iter - 1;
		cudaMemcpy(d_Field, Field.data() + M * N * (time_ind % NFrames), M * N * sizeof(Real), cudaMemcpyHostToDevice);
		break;
	...
	case 6:
		timer.tic();
		WaveIterationAltOMP(FieldAlt.data(), IR.data(), N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, F.data(), b, 64, Threads);
		timer.toc();
	...
	case 7:
		timer.tic();
		WaveIterationKernelAlt << < 1, 1 >> > (d_FieldAlt, d_IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, d_F, 32, b);
		cudaDeviceSynchronize();
		timer.toc();
		checkCudaErrors(cudaGetLastError());
		break;
	...
	if (CalculationMode==6 || CalculationMode == 7)
		dataToColormap2 << < windowGridSize, windowBlockSize >> > ((uchar4*)d_output, windowSize.x, windowSize.y, d_FieldAlt, time_ind  % 2, pow(2.0, intensitySetting), NFrames);
	else 
		dataToColormap << < windowGridSize, windowBlockSize >> >  ((uchar4*)d_output, windowSize.x, windowSize.y, d_Field, (CalculationMode >= 3) ? (time_ind % NFrames) : 0, pow(2.0,intensitySetting),NFrames);
...
void display()
{
	...
	if (CalculationMode == 4 || CalculationMode == 2)
		timerinfo = timer.getInfo(min(timer.N, T) / iter);
	else
		timerinfo = timer.getInfo();
	if (time_ind % T == 0) std::cout << "mean: "<< timerinfo.mean<<" microseconds, Sigma = " << sqrt(timerinfo.dispersion) << 

...
void keyboard(unsigned char key, int x, int y)
{
	...
	//case '0':
	//	CalculationMode = 0;
	//	cudaMemcpy(Field.data() + M * N * (time_ind % 3), d_Field, M * N * sizeof(Real), cudaMemcpyDeviceToHost);
	//	break;
	//case '1':
	//	CalculationMode = 1;
	//	cudaMemcpy(Field.data() + M * N * (time_ind % 3), d_Field, M * N * sizeof(Real), cudaMemcpyDeviceToHost);
	...
	//case '3':
	//	CalculationMode = 3;
	//	cudaMemcpy(d_Field, Field.data() + M * N * (time_ind % 3), M * N * sizeof(Real), cudaMemcpyHostToDevice);
	//	break;
	//case '4':
	//	CalculationMode = 4;
	//	cudaMemcpy(d_Field, Field.data() + M * N * (time_ind % 3), M * N * sizeof(Real), cudaMemcpyHostToDevice);
...
void render()
{
	...
	case 0:
		timer.tic();
		WaveIteration(Field, IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, F, b);
		timer.toc();
		cudaMemcpy(d_Field, Field.data() + M * N * (time_ind % 3), M * N * sizeof(Real), cudaMemcpyHostToDevice);
		break;
	case 1:
		timer.tic();
		WaveIterationOMP(Field, IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, F, b);
		timer.toc();
		cudaMemcpy(d_Field, Field.data() + M * N * (time_ind % 3), M * N * sizeof(Real), cudaMemcpyHostToDevice);
		break;
	case 2:
		timer.tic();
		WaveIterationKernel << < windowGridSize, windowBlockSize >> > (d_Field, d_IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, d_F, b);
	...
	case 3:
		timer.tic();
		MultipleIterationsKernel << < 1, 1 >> > (d_Field, d_IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, d_F, b, iter);
		cudaDeviceSynchronize();
		time_ind += iter - 1;
		timer.toc(iter);
		checkCudaErrors(cudaGetLastError());
	...
	case 4:
		timer.tic();
		WaveIterationKernelSM << < windowGridSize, windowBlockSize,  ((windowBlockSize.x+2) * (windowBlockSize.y+2)*3) * sizeof(Real)>> > (d_Field, d_IR, N, M, time_ind % T0 + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, d_F, b);
		cudaDeviceSynchronize();
		timer.toc();
		checkCudaErrors(cudaGetLastError());
		break;
	...
	dataToColormap << < windowGridSize, windowBlockSize >> >  ((uchar4*)d_output, windowSize.x, windowSize.y, d_Field, (CalculationMode >= 3) ? (time_ind % 3) : 0, pow(2.0,intensitySetting));
	cudaDeviceSynchronize();
	checkCudaErrors(cudaGetLastError());
...
void display()
{
	...
	//sdkStopTimer(&timer);
	//computeFPS();
	TimerInfo timerinfo = timer.getInfo();
	if (time_ind % T == 0) std::cout << "mean: "<< timerinfo.mean<<" microseconds, Sigma = " << sqrt(timerinfo.dispersion) << std::endl;
}
...
void loadDefaultF() {
	// Source
	float f0 = hz / 2;                    /* source dominant frequency, Hz */
...
void benchmark(int begN=1000,int endN=10000, int stepN=1000) {
	...
	for (int sizes = begN; sizes <= endN; sizes += stepN) {
		...
		for (CalculationMode=0; CalculationMode <= 4 ;++CalculationMode)
		{		
			...
			for (time_ind = 0; time_ind < T;) {
				...
				case 1:
					timer0.tic();
					WaveIterationOMP(Field, IR, N, M, time_ind % T + 1, v, d_x, timestep, x_ir, y_ir, x_s, y_s, F, b); 
					timer0.toc(); break;
				case 2:
					timer0.tic();
			...
			if (CalculationMode >=2) checkCudaErrors(cudaGetLastError());
			TimerInfo timerinfo;
			if (CalculationMode == 3)
				timerinfo = timer0.getInfo(min(timer0.N, T) / iter);
			else
				timerinfo = timer0.getInfo();
			std::cout <<"size = "<< sizes << ", calculation mode = " << CalculationMode << ", mean = " << timerinfo.mean << " microseconds, Sigma = " << sqrt(timerinfo.dispersion) << std::endl;
		}
	}
	cudaFree(d_Field);
	cudaFree(d_IR);
...
void RunAndSaveResults(int mode = 0, int writeresults=0) {
	...
	if (mode == 1) CalculationMode = 3;
	if (mode == 2) CalculationMode = 4;
	if (mode == 3) CalculationMode = 5;
	...
	TimerInfo timerinfo;
	//timerinfo = timer0.getInfo();
	if (CalculationMode == 4) timerinfo = timer0.getInfo(1);
	else timerinfo = timer0.getInfo();
	std::cout <<"runSim mode = " << mode << ", mean = " << timerinfo.mean << " microseconds, Sigma = " << sqrt(timerinfo.dispersion) << std::endl;
	std::chrono::high_resolution_clock::time_point end = std::chrono::high_resolution_clock::now();
...
int main(int argc, char** argv) {
	...
	if (filename[0] != '\0') {
		...
		if (data != nullptr) {
			...
			for (int y0 = 0; y0 < N; ++y0) {
				for (int x0 = 0; x0 < M; ++x0) {
					Field(3, y0, x0) = ((Real)tmp(y0, x0)) / 255.0;
				}
			}
			//tmp.free();
		...
		else { Field.resize(4, N, M); Field = 0; Field(3, blitz::Range::all(), blitz::Range::all()) = 1.0;}
		room.close();
	}
	else { Field.resize(4, N, M); Field = 0; Field(3, blitz::Range::all(), blitz::Range::all()) = 1.0; }
	IR.resize(2 * T);
...
//	Real* d_Field;
//	cudaMalloc((void**)&d_Field, M * N * sizeof(Real));
//	cudaMemcpy(d_Field, Field.data(), M*N * sizeof(Real), cudaMemcpyHostToDevice);
